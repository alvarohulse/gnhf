...
Each successful iteration is a separate commit, normally on a dedicated `gnhf/<slug>` branch; `--current-branch` uses the existing branch instead. This maintained fork never pushes user work.
...
1. `cli.ts` decides one of four modes: new branch, resume an existing `gnhf/<slug>` branch, `--current-branch`, or `--worktree` (creates a sibling `<repo>-gnhf-worktrees/<slug>/` checkout). New branch and worktree runs probe numeric suffixes such as `gnhf/<slug>-1` and `<repo>-gnhf-worktrees/<slug>-1/` on collisions; current-branch runs first resume an exact same-prompt `.gnhf/runs/<runId>/` on a clean working tree, otherwise they probe `.gnhf/runs/<runId>-1/` metadata collisions without creating a branch; worktree mode also resumes preserved suffixed worktrees before creating a new one. When resuming with a different prompt, it asks whether to update `prompt.md` and continue the existing run history, start a new branch, or quit; if stdin is piped, that confirmation comes from the controlling terminal before any sleep-prevention re-exec. `setupRun`/`resumeRun` in `src/core/run.ts` create `.gnhf/runs/<runId>/` with `prompt.md`, `notes.md`, `output-schema.json`, `base-commit`, optional `stop-when`, `runtime-limits.json`, `commit-message`, and `gnhf.log`, and locally exclude both `.gnhf/runs/` and `.gnhf/setup-failures/`. Worktree setup failures remain durable and move into the run directory after a successful resume. When a supervisor sets `GNHF_WORKTREE_RECEIPT_PATH`, worktree setup and shutdown publish owner-scoped state receipts atomically.
2. `Orchestrator` (`src/core/orchestrator.ts`) is an `EventEmitter` loop. Each iteration: build prompt via `src/templates/iteration-prompt.ts` (injects current `notes.md`), add recovery instructions when interrupted work or a prior `commitAll` failure needs repair, call `agent.run(...)`, then on success `commitAll` + append to `notes.md`; on failure `resetHard` unless recovery metadata protects the workspace. The user-visible failure, rollback, and runtime-control contracts live in the README's "How It Works"; in code, `commitAll` throws `CommitFailedError` when staging or committing fails, logs `git:commit:failed`, and records repair context for the next iteration, retryable thrown agent errors increment the backoff streak, and `PermanentAgentError` aborts after rollback unless recovery metadata requires preservation, with `lastAgentError` set for the renderer.
...
4. Shutdown path: `SIGINT` routes through `orchestrator.handleInterrupt()`. The first press requests a graceful stop, letting the current iteration finish or ending backoff early; the second requests forced shutdown via `orchestrator.stop()`, and `SIGTERM` does the same. `cli.ts` only keeps the done screen open for aborted runs; graceful stops exit once shutdown cleanup finishes. `getWorktreePreservationReason` gates worktree removal; the README's "Worktree Mode" owns the user-visible preservation contract. After cleanup, `cli.ts` collects final branch/diff stats via `src/core/git.ts` and writes the permanent stdout summary rendered by `src/core/exit-summary.ts`, including an uncommitted-work warning when a commit failure is still pending.
...
- `rovodev.ts` / `opencode.ts`: long-running local HTTP servers managed via `managed-process.ts` (start once, reuse across iterations, close on shutdown). Cleanup failures retain the exact server identity so shutdown can retry the same owner. OpenCode creates a session per iteration, accepts terminal output only when its assistant-message identity has a matching usage receipt, prioritizes streamed provider failures, aborts any unsettled turn before session deletion, and applies a blanket allow rule to avoid prompt blocking.
- `acp.ts`: handles `acp:<target-or-command>` specs through the bundled `acpx` runtime and registry. It keeps a persistent per-run session keyed by run ID under `.gnhf/runs/<runId>/acp-sessions`, embeds the output schema in the prompt, parses only output text deltas as final JSON, and records ACP lifecycle events in `gnhf.log`. A cleanly cancelled and settled turn leaves the runtime reusable after a stream error; incomplete turn cleanup attempts runtime close and fails closed. The README's [Runtime caps](./README.md#how-it-works) and [Agents](./README.md#agents) sections own user-visible usage availability and limit behavior. Path and arg overrides are native-agent-only; ACP targets are customized via `acpRegistryOverrides` in config (a target-name -> spawn-command map fed into acpx's agent registry) or by passing a raw ACP server command directly after `acp:`. Raw command specs are redacted to `acp:custom`/`custom` in debug logs, errors, and telemetry. The e2e suite exercises the full wire path against the `acp-mock` package registered through that same override mechanism.
...
- `stream-utils.ts`: shared JSONL parsing and `AbortSignal` wiring. `managed-process.ts` owns identity-scoped shutdown: group-aware Unix adapters use a generation-stable supervisor in a dedicated process group unless `GNHF_SUPERVISED_PROCESS_GROUP=1` keeps them inside an external supervisor group, while Windows adapters keep a stable supervisor tree root. Explicit Windows shutdown while that root is live requires tracked `/T /F` cleanup and exact root closure; incomplete cleanup fails closed. If the target or an intermediate shell exits naturally first, descendant ownership is no longer provable, so the primary result or error remains authoritative while gnhf attempts best-effort cleanup and records it as unverified in the debug log. It never claims descendant cleanup in that state. A Job Object-backed host is the deferred hard-cleanup enhancement. Direct-child adapters signal only their spawned PID. The same setting keeps Linux sleep-inhibitor re-execution in that supervisor group. When touching agent streaming or cleanup, start with these helpers.
...
Loads `~/.gnhf/config.yml` (bootstrapped on first run). CLI flags override config; the README's [CLI Reference](./README.md#cli-reference) owns per-run flag and resume-persistence behavior. `agent` accepts native agent names plus `acp:<target-or-command>` specs; `agentPathOverride` and `agentArgsOverride` are native-agent-only, and paths resolve relative to `~/.gnhf/` with `~` expansion. `acpRegistryOverrides` maps ACP target names to spawn commands, threaded into acpx's agent registry so `acp:<name>` resolves against the override map first - useful for pinning a custom or local build of an ACP agent while keeping logs/telemetry on the named target. Raw ACP command specs can also be passed directly and are logged as `acp:custom`. `commitMessage.preset: conventional` adds commit-message fields to the output schema/prompt and changes successful-iteration commit subjects; the resolved convention is persisted per run so resume does not silently switch formats after config changes.
