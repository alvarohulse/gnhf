...
- `stream-utils.ts`: shared JSONL parsing and `AbortSignal` wiring. For agents that manage process trees, `managed-process.ts` owns identity-scoped child signaling; those agents detach into dedicated process groups unless `GNHF_SUPERVISED_PROCESS_GROUP=1` keeps them inside an external supervisor group. The same setting keeps Linux sleep-inhibitor re-execution in that supervisor group. When touching agent streaming or cleanup, start with these helpers.
