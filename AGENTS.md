...
2. `Orchestrator` (`src/core/orchestrator.ts`) is an `EventEmitter` loop. Each iteration: build prompt via `src/templates/iteration-prompt.ts` (injects current `notes.md`), add recovery instructions when interrupted work or a prior `git commit` failure needs repair, call `agent.run(...)`, then on success `commitAll` + append to `notes.md`; on failure `resetHard` unless recovery metadata protects the workspace. The user-visible failure, rollback, and runtime-control contracts live in the README's "How It Works"; in code, `commitAll` throws `CommitFailedError`, logs `git:commit:failed`, and records repair context for the next iteration, retryable thrown agent errors increment the backoff streak, and `PermanentAgentError` aborts after rollback unless recovery metadata requires preservation, with `lastAgentError` set for the renderer.
