...
- `stream-utils.ts`: shared JSONL parsing and `AbortSignal` wiring. `managed-process.ts` owns identity-scoped shutdown: group-aware Unix adapters use a generation-stable supervisor in a dedicated process group unless `GNHF_SUPERVISED_PROCESS_GROUP=1` keeps them inside an external supervisor group, while Windows adapters keep a stable supervisor tree root until tracked `/T /F` cleanup completes. If the Windows target or an intermediate shell exits first, descendant ownership is no longer provable; adapters fail closed and preserve the workspace. A Job Object-backed host is the deferred hard-cleanup enhancement for that case. Direct-child adapters signal only their spawned PID. Adapters settle only after owned cleanup completes; incomplete cleanup is surfaced so the workspace is preserved. The same setting keeps Linux sleep-inhibitor re-execution in that supervisor group. When touching agent streaming or cleanup, start with these helpers.
