...
- `stream-utils.ts`: shared JSONL parsing and `AbortSignal` wiring. `managed-process.ts` owns identity-scoped Unix shutdown: group-aware adapters use a generation-stable supervisor in a dedicated process group unless `GNHF_SUPERVISED_PROCESS_GROUP=1` keeps them inside an external supervisor group, while direct-child adapters signal only their spawned PID. Adapters settle only after owned cleanup completes; incomplete cleanup is surfaced so the workspace is preserved. Windows adapters terminate the tree rooted at the exact spawned PID. The same setting keeps Linux sleep-inhibitor re-execution in that supervisor group. When touching agent streaming or cleanup, start with these helpers.
